...
export default function SettingsPanel({ onClose }: Props) {
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input type={show ? 'text' : 'password'} value={value} onChange={e => onChange(e.target.value)}
          ...
          className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-11 pr-11 text-sm focus:border-emerald-500/40 focus:outline-none placeholder:text-white/20 text-white transition-colors" />
        <button type="button" onClick={toggle} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/20 hover:text-white/60 transition-colors">
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] flex flex-col bg-[#0e1a14] border border-white/10 rounded-3xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 flex-shrink-0 border-b border-white/5">
          <h2 className="text-base font-black text-white uppercase tracking-widest">{t.profileSettings}</h2>
          <button onClick={onClose} className="p-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white/40 hover:text-white transition-all cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* Tabs */}
        <div className="flex bg-white/5 mx-6 mt-4 rounded-xl p-1 gap-1 flex-shrink-0">
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-[11px] font-black uppercase tracking-widest rounded-lg transition-all cursor-pointer ${activeTab === id ? 'bg-white/10 text-white' : 'text-white/30 hover:text-white'}`}>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar px-6 py-4">
          ...
          {activeTab === 'profile' && (
            <form onSubmit={handleProfileSave} className="space-y-4">
              {/* Avatar */}
              <div className="flex items-center gap-5 p-5 bg-white/[0.03] border border-white/5 rounded-2xl">
                <div className="relative group cursor-pointer flex-shrink-0" onClick={() => fileRef.current?.click()}>
                  <div className="w-16 h-16 rounded-2xl overflow-hidden bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center border border-white/10">
                    {avatarPreview
                ...
                <div>
                  ...
                  {user?.tenantName && (
                    <div className="flex items-center gap-1.5 mt-1">
                      <Building2 className="w-3 h-3 text-emerald-400/70" />
                      <span className="text-xs text-emerald-400/70 font-medium">{user.tenantName}</span>
                    </div>
                  )}
                  <button type="button" onClick={() => fileRef.current?.click()}
                    className="mt-2 flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/50 hover:text-white transition-all cursor-pointer">
              ...
              {profileSuccess && (
                <div className="flex items-center gap-2 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-400 text-sm">
              ...
              <div className="space-y-3 p-5 bg-white/[0.03] border border-white/5 rounded-2xl">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={fullName} onChange={e => setFullName(e.target.value)}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-11 pr-4 text-sm focus:border-emerald-500/40 focus:outline-none placeholder:text-white/20 text-white transition-colors" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-white/40">{t.emailAddress}</label>
                  <div className="relative">
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">{ROLE_LABELS[user?.role ?? ''] ?? user?.role}</span>
                </div>
              </div>
          ...
          {activeTab === 'security' && (
            <form onSubmit={handlePasswordChange} className="space-y-4">
              <p className="text-sm text-white/40">{t.manageSecurity}</p>
                  ...
                  <div className="space-y-1 pt-1">
                    {[
                      ...
                      <div key={label} className={`flex items-center gap-2 text-xs ${ok ? 'text-emerald-400' : 'text-white/30'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${ok ? 'bg-emerald-400' : 'bg-white/20'}`} />
                        {label}
                      </div>
                    ))}
                  </div>
              ...
              <button type="submit" disabled={secLoading || !currentPw || !newPw || !confirmPw}
                ...
                {secLoading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <KeyRound className="w-4 h-4" />}
          ...
          {activeTab === 'preferences' && (
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button key={lng.code} onClick={() => setLanguage(lng.code as any)}
                      className={`flex items-center justify-between px-4 py-3 rounded-xl border text-sm transition-all cursor-pointer ${lang === lng.code ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-400' : 'bg-white/5 border-white/10 text-white/50 hover:text-white hover:border-white/20'}`}>
              ...
              {/* Account info */}
              <div className="p-5 bg-white/[0.03] border border-white/5 rounded-2xl space-y-3">
                <span className="text-sm font-bold text-white">{t.accountInfo}</span>
              ...
              <button onClick={() => logout()}
                className="flex items-center gap-2 px-5 py-3 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 hover:border-red-500/40 text-red-400 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer">
