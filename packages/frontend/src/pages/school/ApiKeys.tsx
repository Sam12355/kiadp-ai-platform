    ...
    <div className="animate-fade-in max-w-5xl mx-auto space-y-8">
      ...
      {createdKey && (
        <div className="glass rounded-[1.5rem] p-6 border border-emerald-500/30 space-y-3">
          ...
          <button onClick={() => setCreatedKey(null)} className="text-[10px] text-ink-faint hover:text-ink-soft uppercase tracking-widest font-black transition-colors cursor-pointer">
            Dismiss
          </button>
        </div>
      )}
      ...
      <div className="glass rounded-[1.5rem] p-6 space-y-4">
        <h2 className="text-xs font-black uppercase tracking-widest text-ink-mute">Create New Key</h2>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 bg-raised border border-line rounded-2xl text-ink placeholder:text-ink-faint text-sm focus:outline-none focus:border-blue-500/50 transition-colors"
          />
          <button
      ...
      {isLoading ? (
        ...
        <div className="glass rounded-[1.5rem] p-12 text-center">
          <Key className="w-12 h-12 text-ink-faint mx-auto mb-4" />
          <p className="text-ink-mute font-medium">No API keys yet. Create one above.</p>
        </div>
      ) : (
          ...
          {keys.map((k) => (
            <div key={k.id} className="glass rounded-[1.5rem] p-5 flex items-center gap-4 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink-faint font-mono mt-0.5">{k.keyPrefix}••••••••••••</p>
                <div className="flex items-center gap-3 mt-1 text-[10px] text-ink-faint uppercase tracking-widest">
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  title="Rotate key"
                  className="p-2 text-ink-faint hover:text-blue-600 transition-colors cursor-pointer"
                >
                ...
                <button
                  ...
                  title="Revoke key"
                  className="p-2 text-ink-faint hover:text-red-600 transition-colors cursor-pointer"
                >
