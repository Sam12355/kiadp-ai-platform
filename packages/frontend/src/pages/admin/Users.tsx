  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-ink tracking-tight uppercase" style={{ fontFamily: 'var(--font-heading)' }}>{t.identityManagement}</h1>
          <p className="text-ink-mute mt-2 font-medium">{t.manageSecurity}</p>
        </div>
        
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-72 group">
            <Search className="absolute ltr:left-4 rtl:right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-mute group-focus-within:text-emerald-600 transition-colors" />
            <input
              type="text"
              placeholder={t.filterNames}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-raised border border-line rounded-2xl py-3 ltr:pl-12 rtl:pr-12 ltr:pr-4 rtl:pl-4 text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:border-emerald-500/40 transition-all font-medium"
            />
          </div>
        ...
        <div className="glass rounded-[2rem] overflow-hidden border border-amber-500/20 shadow-2xl">
          ...
          <div className="divide-y divide-line-soft">
            {pendingUsers.map((user) => (
              <div key={user.id} className="px-8 py-5 flex items-center gap-4 hover:bg-raised transition-colors">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center text-white font-black border border-line shadow-lg">
                  {user.fullName.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-ink leading-none">{user.fullName}</p>
                  <div className="flex items-center gap-1.5 text-[10px] font-bold text-ink-mute mt-1 uppercase tracking-tight">
                    <Mail className="w-3 h-3" />
                    {user.email}
                  </div>
                </div>
                <div className="flex items-center gap-1.5 text-[10px] font-bold text-ink-faint uppercase tracking-widest me-4">
      ...
      <div className="glass rounded-[2rem] overflow-hidden border border-line-soft shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left rtl:text-right text-sm">
            <thead className="uppercase text-ink-mute bg-raised border-b border-line-soft">
            ...
            <tbody className="divide-y divide-line-soft">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-12 text-center text-ink-mute italic font-black uppercase tracking-widest text-[10px]">
                    {t.noUsersFound}
                  </td>
                </tr>
              ) : filteredUsers?.map((user) => (
                <tr key={user.id} className="group hover:bg-raised transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-800 flex items-center justify-center text-white text-lg font-black border border-line shadow-2xl overflow-hidden group-hover:scale-105 transition-transform">
                        {user.avatarUrl ? (
                      ...
                      <div>
                        <p className="font-bold text-ink leading-none">{user.fullName}</p>
                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-ink-mute mt-1.5 uppercase tracking-tight">
                          <Mail className="w-3 h-3" />
                          {user.email}
                        </div>
                      </div>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-1.5 text-[10px] font-bold text-ink-mute uppercase tracking-widest">
                      <Calendar className="w-3 h-3" />
                      {new Date(user.createdAt).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US')}
                    </div>
                  </td>
