...
const Sep = () => <div className="w-px h-5 bg-overlay mx-0.5 flex-shrink-0" />;
...
function TextEntryAddModal({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'min-h-[240px] outline-none text-sm text-ink leading-relaxed prose prose-invert max-w-none focus:outline-none',
      },
    },
  });
  ...
  return (
    <Portal>
      ...
      >
        ...
        {/* Panel */}
        <div
          className="w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-[3rem] p-10 shadow-[0_32px_128px_rgba(0,0,0,0.35)] border border-line-strong relative z-10 animate-fade-in mx-4"
        ...
        >
          ...
          <div className="flex justify-between items-center mb-8 border-b border-line pb-6 relative z-20">
            <div className="flex items-center gap-4 text-emerald-700">
              ...
              <h2 className="text-2xl font-black text-ink uppercase tracking-tighter" style={{ fontFamily: 'var(--font-heading)' }}>
                {t.textualKnowledge}
              </h2>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8 relative z-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.25em] text-ink-mute ml-1">{t.document}</label>
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.25em] text-ink-mute ml-1">{t.category}</label>
            ...
            {/* Editor */}
            <div className="space-y-3">
              <label className="block text-[10px] font-black uppercase tracking-[0.25em] text-ink-mute ml-1">{t.contentBody}</label>
              <div className="border border-line rounded-2xl bg-raised overflow-hidden">
                {/* Toolbar Row 1 */}
                <div className="flex items-center gap-0.5 p-2 border-b border-line-soft flex-wrap">
                ...
                {/* Toolbar Row 2 */}
                <div className="flex items-center gap-2 p-2 border-b border-line-soft flex-wrap">
                  <select value={curFontFamily}
                    ...
                    className="bg-raised border border-line rounded-lg text-[11px] text-ink-soft px-2 py-1.5 focus:outline-none focus:border-emerald-500/40 cursor-pointer max-w-[130px]">
                  ...
                  <select value={curFontSize}
                    ...
                    className="bg-raised border border-line rounded-lg text-[11px] text-ink-soft px-2 py-1.5 focus:outline-none focus:border-emerald-500/40 cursor-pointer w-[72px]">
                    <option value="">Size</option>
                    {FONT_SIZES.map(s => <option key={s} value={s}>{s.replace('px', '')}</option>)}
                  </select>
                  <button type="button" title={t.textColor} onClick={() => colorInputRef.current?.click()}
                    className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg border border-line hover:bg-raised transition-all">
                    <Palette className="w-3.5 h-3.5 text-ink-mute" />
                    <div className="w-4 h-3.5 rounded-[3px] border border-line-strong" style={{ backgroundColor: currentColor }} />
                  </button>
                  ...
                  <div className="relative">
                    ...
                    {showImageMenu && (
                      <div className="absolute top-full start-0 mt-1 z-50 bg-raised border border-line rounded-xl p-3 shadow-2xl w-72">
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-[9px] font-black uppercase tracking-wider text-ink-mute">{t.insertImage}</p>
                          <button type="button" onClick={() => setShowImageMenu(false)}><X className="w-3.5 h-3.5 text-ink-mute hover:text-ink" /></button>
                        </div>
                        <div className="flex gap-2">
                          <input type="text" placeholder="https://..." value={imageUrl} onChange={(e) => setImageUrl(e.target.value)}
                            ...
                            className="flex-1 bg-raised border border-line rounded-lg px-2.5 py-2 text-xs text-ink placeholder:text-ink-faint focus:outline-none focus:border-emerald-500/40" />
                        ...
                        <button type="button" onClick={() => fileInputRef.current?.click()}
                          className="w-full mt-2 py-2 text-[10px] font-bold text-ink-mute hover:text-ink border border-line hover:border-line-strong rounded-lg transition-all">
                          📁 {t.uploadFile}
                        </button>
                      </div>
                    )}
              ...
              <div className="text-right text-[10px] font-bold text-ink-mute uppercase tracking-widest">
                {charCount} {t.charsLabel}
              </div>
            </div>

            {/* Footer */}
            <div className="pt-6 flex justify-end gap-4 border-t border-line">
              <button type="button" onClick={onClose}
                className="px-8 py-4 rounded-2xl text-sm font-bold text-ink-mute hover:text-ink transition-all uppercase tracking-widest">
                {t.discard}
              </button>
              <button type="submit" disabled={loading}
                ...
                {loading ? <div className="w-5 h-5 border-2 border-line-strong border-t-white rounded-full animate-spin" /> : <Save className="w-5 h-5" />}
  ...
  return (
    <Portal>
      ...
      >
        ...
        />
        <div
          className="w-full max-w-lg rounded-[3rem] p-12 shadow-[0_32px_128px_rgba(0,0,0,0.3)] border border-line-strong relative z-10 animate-fade-in overflow-visible mx-4"
          style={{ background: 'var(--t-glass)', backdropFilter: 'blur(20px)', position: 'relative', zIndex: 10 }}
        >
...
export default function InsertKnowledge() {
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            <Search className="absolute ltr:left-3 rtl:right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-mute" />
            <input type="text" placeholder={t.searchLibrary} value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-raised border border-line rounded-xl py-2.5 ltr:pl-10 rtl:pr-10 ltr:pr-4 rtl:pl-4 text-sm text-ink focus:border-emerald-500/50 transition-all focus:outline-none" />
          </div>
      ...
      {/* ── Entries table ──────────────────────────────────────────────────── */}
      <div className="glass rounded-[2rem] overflow-hidden border border-line-soft shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left rtl:text-right text-sm">
            <thead className="text-[10px] uppercase font-black tracking-widest text-ink-mute bg-raised border-b border-line-soft">
              <tr>
            ...
            <tbody className="divide-y divide-line-soft">
              {tableLoading ? (
                <tr><td colSpan={5} className="px-8 py-12 text-center text-ink-mute italic">{t.thinking}</td></tr>
              ) : filteredEntries.length === 0 ? (
                <tr><td colSpan={5} className="px-8 py-12 text-center text-ink-mute italic font-black uppercase tracking-widest text-[10px]">{t.noTextEntries}</td></tr>
                ...
                <tr key={entry.id} className="group hover:bg-raised transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="max-w-[240px]">
                        <p className="font-bold text-ink truncate" title={entry.title}>{entry.title}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6 text-[10px] font-bold text-ink-mute uppercase tracking-widest">
                  ...
                  <td className="px-8 py-6 text-right rtl:text-left">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setEditingDoc(entry)}
                        className="p-2.5 text-ink-mute hover:text-amber-600 hover:bg-amber-400/10 rounded-xl transition-all"
                        title={t.editMetadata}>
                        <Edit3 className="w-5 h-5" />
                      </button>
                      <button onClick={() => setDeletingId(entry.id)}
                        className="p-2.5 text-ink-mute hover:text-red-600 hover:bg-red-400/10 rounded-xl transition-all"
                        title={t.delete}>
