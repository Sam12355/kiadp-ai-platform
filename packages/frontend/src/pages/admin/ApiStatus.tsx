...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === 'unconfigured')
    return (
      <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-ink-mute">
...
function ProgressBar({ label, usedPercent, left, right }: { label: string; usedPercent: number; left: string; right: string }) {
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-ink-mute">
        <span>{label}</span>
        <span className="text-ink">{left} / {right}</span>
      </div>
      <div className="h-1.5 rounded-full bg-overlay overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-500 ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <div className="text-right text-[10px] font-bold text-ink-mute">{pct.toFixed(1)}{t.percentUsed}</div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between mb-5">
      <div className="flex items-center gap-3">
        <div className={`w-9 h-9 rounded-xl ${color} flex items-center justify-center border border-line`}>
          <Icon className="w-4.5 h-4.5 text-ink" />
        </div>
        <p className="text-sm font-black text-ink uppercase tracking-wide">{name}</p>
      </div>
      {badge}
    </div>
  );
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block bg-raised border border-line text-[10px] font-bold text-ink-mute rounded-md px-2 py-0.5">{children}</span>
  );
}
      ...
      {d.status === 'online' && d.storage && d.bandwidth && d.transformations && (
        <>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[10px] font-black uppercase tracking-widest text-ink-mute">{t.planLabel}</span>
            <Tag>{d.plan}</Tag>
...
function OpenAICard({ d, period }: { d: OpenAIInfo; period?: string }) {
  ...
  const t = translations[lang];
  return (
    <div className="glass rounded-[1.5rem] p-6 border border-line-soft space-y-3">
      ...
      {d.creditBalance !== undefined ? (
        <div className="bg-raised rounded-xl p-4 border border-line-soft">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-mute mb-2">{t.creditBalance}</p>
          <div className="flex items-end gap-1.5">
            <span className="text-3xl font-black text-ink tracking-tighter" style={{ fontFamily: 'var(--font-heading)' }}>
              ${d.creditBalance.toFixed(2)}
            </span>
            <span className="text-[10px] font-black text-ink-mute mb-1">{t.usdRemaining}</span>
          </div>
      ...
      <div>
        <p className="text-[10px] font-black uppercase tracking-widest text-ink-mute mb-2">{t.selfTrackedUsageLabel}</p>
...
function GeminiCard({ d, period }: { d: GeminiInfo; period?: string }) {
  ...
  return (
    <div className="glass rounded-[1.5rem] p-6 border border-line-soft space-y-3">
      ...
      {d.chatModels && (
        <div className="space-y-1.5">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-mute">{t.chatFallbackChain}</p>
      ...
      {d.voiceModel && (
        <div className="space-y-1.5 pt-1">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-mute">{t.voiceModelLabel}</p>
...
function GroqCard({ d, period }: { d: GroqInfo; period?: string }) {
  ...
  return (
    <div className="glass rounded-[1.5rem] p-6 border border-line-soft space-y-3">
      ...
      {d.chatModels && (
        <div className="space-y-1.5">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-mute">{t.chatFallbackModels}</p>
          <div className="flex flex-col gap-1">
            {d.chatModels.map(m => <Tag key={m}>{m}</Tag>)}
          </div>
        </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between bg-raised rounded-xl px-3 py-2.5 border border-line-soft">
      <span className="text-[10px] font-black uppercase tracking-widest text-ink-mute">{label}</span>
      <span className="text-[11px] font-bold text-ink font-mono">{value}</span>
    </div>
  );
}
...
function SelfTrackedBlock({ data, period, emptyText }: { data?: SelfTracked | null; period?: string; emptyText?: string }) {
  ...
  if (!data || data.requests === 0) return (
    <p className="text-[10px] text-ink-mute italic pt-1">{emptyText ?? t.noUsageRecorded}</p>
  );
  const modelEntries = Object.entries(data.byModel).sort((a, b) => b[1].requests - a[1].requests);
  return (
    <div className="space-y-2 pt-1">
      {period && <p className="text-[10px] font-bold text-ink-mute uppercase tracking-wider">{period}</p>}
      <div className="grid grid-cols-2 gap-2">
        {[
          { label: t.requestsUsed, val: fmtNum(data.requests) },
          { label: t.modelsActive, val: fmtNum(modelEntries.length) },
        ].map(({ label, val }) => (
          <div key={label} className="bg-raised rounded-xl p-2.5 border border-line-soft text-center">
            <p className="text-base font-black text-ink tracking-tighter" style={{ fontFamily: 'var(--font-heading)' }}>{val}</p>
            <p className="text-[9px] font-black uppercase tracking-widest text-ink-mute mt-0.5">{label}</p>
          </div>
        ))}
      </div>
      {modelEntries.length > 0 && (
        <div className="space-y-1 pt-1">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-mute">{t.usageSplit}</p>
            ...
            return (
              <div key={mdl} className="space-y-1">
                <div className="flex items-center justify-between text-[10px]">
                  <span className="font-bold text-ink-mute font-mono truncate max-w-[150px]">{mdl}</span>
                  <span className="text-ink font-bold">{fmtNum(stats.requests)} / {fmtNum(data.requests)} ({pct.toFixed(1)}%)</span>
                </div>
                <div className="h-1.5 rounded-full bg-overlay overflow-hidden">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-ink-mute hover:text-ink transition-colors disabled:opacity-40"
        >
